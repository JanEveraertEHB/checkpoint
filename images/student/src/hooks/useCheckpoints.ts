...
export function useCheckpoints(classroomUuid: string | undefined) {
  ...
  const fetchStudentProgress = useCallback(async (studentUuid: string) => {
    if (!classroomUuid || !studentUuid) return
    try {
      const response = await getStudentProgress(classroomUuid, studentUuid)
      setStudentProgress(response.data.checkpoints || [])
      setNextCheckpoint(response.data.next_checkpoint || null)
    } catch (err: any) {
      // Handle API errors gracefully - the backend might not be ready yet
      if (err.response?.status === 500) {
        console.warn('Student progress API returned 500 - backend may need to be updated for student app')
      } else {
        console.error('Error fetching student progress:', err)
      }
      // Set empty state so the app continues to work
      setStudentProgress([])
